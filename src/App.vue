	...
	<div id="app" v-cloak>
		<van-notice-bar color="#1989fa" background="#ecf9ff" :scrollable="false">
			<van-swipe
				...
				:touchable="false"
			>
				<van-swipe-item v-for="(item, index) in currentAction" :key="index">{{
					item.name
				}}</van-swipe-item>
		...
		<div class="btn-operate">
			<van-button
				...
				v-if="isCameraOpen && !inProgress"
				type="info"
				native-type="button"
				>开始录制
			</van-button>
			<van-button v-if="isCameraOpen && inProgress" type="danger" native-type="button" disabled
				>录制中</van-button
			>
			<van-button
				@click="neuAufnehmen"
	...
	methods: {
		...
		drawResults(faces, ctx) {
			faces.forEach((faceItem) => {
				...
				const faceProportion = this.GetPercent(
					faceItem.box.width * faceItem.box.height,
					this.width * this.height
				);
				this.$refs['faceProportion'].innerHTML = `10-50之间,当前距离:<b>${Math.round(
					faceProportion
				)}</b>`;
		...
		isShakingHisHead(face, ctx) {
			...
			const diff = this.GetPercent(
				leftProportion - rightProportion,
				leftProportion + rightProportion
			);
